...
void AnalyticalKinSolver::PrintTransform(Stream& serialPort, Eigen::Isometry3f g) {
    ...
    serialPort.print(g(3,0),4); serialPort.print(" "); serialPort.print(g(3,1),4); serialPort.print(" "); serialPort.print(g(3,2),4); serialPort.print(" "); serialPort.println(g(3,3),4); 
}
...
Eigen::Isometry3f AnalyticalJacobians::ForwardKinematicsTCP(float *q) {
    _g_tcp_sp = twistExp(_robot_def->active_twists[0], *(q+0) ) * _Pi[0] * twistExp(_robot_def->active_twists[1], *(q+1) ) * _Pi[1] * twistExp(_robot_def->active_twists[2], *(q+2) ) * _robot_def->gst0;
    return _g_tcp_sp;
}
...
void AnalyticalJacobians::ToolJacobian(float *q, char jacobian_char_id) {
    ...
    switch (jacobian_char_id)
    {
        ...
        // The spatial jacobian and the spatial TCP position vector (relative to {S) frame are used
            ...
            row0.x() = _Jsp(0,0) - _Jsp(5,0)*_p_tcp_sp.y() + _Jsp(4,0)*_p_tcp_sp.z() ; 
            row0.y() = _Jsp(0,1) - _Jsp(5,1)*_p_tcp_sp.y() + _Jsp(4,1)*_p_tcp_sp.z() ; 
            row0.z() = _Jsp(0,2) - _Jsp(5,2)*_p_tcp_sp.y() + _Jsp(4,2)*_p_tcp_sp.z() ; 

            row1.x() = _Jsp(1,0) + _Jsp(5,0)*_p_tcp_sp.x() - _Jsp(3,0)*_p_tcp_sp.z() ; 
            row1.y() = _Jsp(1,1) + _Jsp(5,1)*_p_tcp_sp.x() - _Jsp(3,1)*_p_tcp_sp.z() ; 
            row1.z() = _Jsp(1,2) + _Jsp(5,2)*_p_tcp_sp.x() - _Jsp(3,2)*_p_tcp_sp.z() ; 

            row2.x() = _Jsp(2,0) - _Jsp(4,0)*_p_tcp_sp.x() + _Jsp(3,0)*_p_tcp_sp.y() ; 
            row2.y() = _Jsp(2,1) - _Jsp(4,1)*_p_tcp_sp.x() + _Jsp(3,1)*_p_tcp_sp.y() ; 
            row2.z() = _Jsp(2,2) - _Jsp(4,2)*_p_tcp_sp.x() + _Jsp(3,2)*_p_tcp_sp.y() ; 
            break;
        case 'b':
